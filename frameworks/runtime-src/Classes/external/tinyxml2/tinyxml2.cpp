...
// added by LiXizhi
static const int NUM_ENTITIES_READER = 6;
static const Entity entities_reader[NUM_ENTITIES_READER] = {
	{ "nbsp", 4, ' ' },  // added by LiXizhi. 
	{ "quot", 4, DOUBLE_QUOTE },
	{ "amp", 3, '&' },
	{ "apos", 4, SINGLE_QUOTE },
	{ "lt", 2, '<' },
                ...
                if ( (_flags & NEEDS_NEWLINE_NORMALIZATION) && *p == CR ) {
                    ...
                    *q++ = LF;
                    ...
                    if ( *(p+1) == '#' ) {
                        ...
                        int i=0;
                        for(; i<NUM_ENTITIES_READER; ++i ) {
							if (strncmp(p + 1, entities_reader[i].pattern, entities_reader[i].length) == 0
								&& *(p + entities_reader[i].length + 1) == ';') {
                                // Found an entity convert;
								*q = entities_reader[i].value;
                                ++q;
								p += entities_reader[i].length + 2;
                                break;
                            }
                        }
                        if ( i == NUM_ENTITIES ) {
...
#pragma warning (pop)
    ...
    if ( XMLUtil::StringEqual( p, xmlHeader, xmlHeaderLen ) ) {
		...
		// Fixed by Xizhi 2014.9.20: this will allow us to parse <%=%> in the code, which is needed for MCML. 
		if (*(p + 1) != '%')
		{
...
char* XMLAttribute::ParseDeep( char* p, bool processEntities )
{
    ...
    if ( *p != '\"' && *p != '\'' ) {
        // fixed by Xizhi 2014.9.21: this will allow non-strict xml attribute like <a zorder=2000/>, which is compatible with tinyxml 1.0. 
		char* start = p;	// fixme: hides a member
		int strFlags = processEntities ? StrPair::ATTRIBUTE_VALUE : StrPair::ATTRIBUTE_VALUE_LEAVE_ENTITIES;
		// Inner loop of text parsing.
		while (*p) {
			if (!XMLUtil::IsAlphaNum(*p)) {
				_value.Set(start, p, strFlags);
				return p;
			}
			++p;
		}
...
XMLError XMLDocument::Parse( const char* p, size_t len )
{
	...
	// fix by Xizhi: skip UTF8 BOM header. 
	ParseDeep(_charBuffer + (p - _charBuffer), 0);
    return _errorID;
}

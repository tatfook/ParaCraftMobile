...
: ${BOOST_LIBS:="atomic chrono date_time system signals filesystem regex thread iostreams"}
...
#    ./b2 -j16 --build-dir=iphone-build --stagedir=iphone-build/stage --prefix=$PREFIXDIR toolset=darwin architecture=arm target-os=iphone macosx-version=iphone-${IPHONE_SDKVERSION} define=_LITTLE_ENDIAN link=static install
    ...
    echo building Boost for OSX i386_x64
    ./b2 -j16 --build-dir=osx-build/i386_x64 --stagedir=osx-build/i386_x64/stage toolset=clang cxxflags="-std=c++11 -stdlib=libc++ -arch i386 -arch x86_64" linkflags="-stdlib=libc++" link=static threading=multi macosx-version=${OSX_SDKVERSION} stage
